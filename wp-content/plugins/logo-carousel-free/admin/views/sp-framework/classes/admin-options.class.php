...
if ( ! class_exists( 'SPLC_FREE_Options' ) ) {
	class SPLC_FREE_Options extends SPLC_FREE_Abstract {
		...
		public function add_options_html() {
			...
			echo '<div class="splogocarousel-form-result splogocarousel-form-success ' . esc_attr( $notice_class ) . '">' . $notice_text . '</div>';
			...
			if ( $show_buttons ) {
				echo '<h1><img src="' . SP_LC_URL . 'admin/assets/images/lc-icon-color.svg">' . $this->args['framework_title'] . '</h1>';
			} else {
				echo '<h1 class="export-import"><img src="' . SP_LC_URL . 'admin/assets/images/import-export.svg">' . $this->args['framework_title'] . '</h1>';
			}
			echo '</div>';
			...
			if ( $has_nav ) {
				...
				foreach ( $this->pre_tabs as $tab ) {
					...
					if ( ! empty( $tab['subs'] ) ) {
						...
						echo '<a href="#tab=' . esc_attr( $tab_id ) . '" data-tab-id="' . esc_attr( $tab_id ) . '" class="splogocarousel-arrow">' . $tab_icon . $tab['title'] . $tab_error . '</a>';
						...
						echo '<li class="splogocarousel-tab-item"><a href="#tab=' . esc_attr( $tab_id ) . '" data-tab-id="' . esc_attr( $tab_id ) . '">' . $tab_icon . $tab['title'] . $tab_error . '</a></li>';
			...
			foreach ( $this->pre_sections as $section ) {
				...
				echo ( $has_nav ) ? '<div class="splogocarousel-section-title"><h3>' . $section_icon . $section_title . '</h3></div>' : '';
				echo ( ! empty( $section['description'] ) ) ? '<div class="splogocarousel-field splogocarousel-section-description">' . $section['description'] . '</div>' : '';
			...
			echo ( $has_nav && $nav_type === 'normal' ) ? '<div class="splogocarousel-nav-background"></div>' : '';
			...
			if ( ! empty( $this->args['show_footer'] ) ) {
				...
				echo ( ! empty( $this->args['footer_text'] ) ) ? '<div class="splogocarousel-copyright">' . $this->args['footer_text'] . '</div>' : '';
