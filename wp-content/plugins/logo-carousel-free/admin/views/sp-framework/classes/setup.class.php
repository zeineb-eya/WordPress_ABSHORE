...
if ( ! class_exists( 'SPLC' ) ) {
	class SPLC {
		...
		public function __construct() {
			...
			add_action( 'wp_head', array( 'SPLC', 'add_custom_css' ), 80 );
			add_filter( 'admin_body_class', array( 'SPLC', 'add_admin_body_class' ) );
		}
		...
		public static function setup() {
			...
			$params = array();
			if ( class_exists( 'SPLC_FREE_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
				foreach ( self::$args['customize_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SPLC_FREE_Customize_Options::instance( $key, $params );
					}
				}
			}
			...
			$params = array();
			if ( class_exists( 'SPLC_FREE_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
				foreach ( self::$args['nav_menu_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SPLC_FREE_Nav_Menu_Options::instance( $key, $params );
					}
				}
			}
			...
			$params = array();
			if ( class_exists( 'SPLC_FREE_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
				foreach ( self::$args['profile_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SPLC_FREE_Profile_Options::instance( $key, $params );
					}
				}
			}
			...
			$params = array();
			if ( class_exists( 'SPLC_FREE_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
				...
				foreach ( self::$args['taxonomy_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SPLC_FREE_Taxonomy_Options::instance( $key, $params );
					}
				}
			}

			// Setup widget option framework
			if ( class_exists( 'SPLC_FREE_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
				$wp_widget_factory = new WP_Widget_Factory();
				foreach ( self::$args['widget_options'] as $key => $value ) {
					if ( ! isset( self::$inited[ $key ] ) ) {

						self::$inited[ $key ] = true;
						$wp_widget_factory->register( SPLC_FREE_Widget::instance( $key, $value ) );
					}
				}
			}
			...
			$params = array();
			if ( class_exists( 'SPLC_FREE_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
				foreach ( self::$args['comment_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SPLC_FREE_Comment_Metabox::instance( $key, $params );
					}
				}
			}
			...
			$params = array();
			if ( class_exists( 'SPLC_FREE_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
				foreach ( self::$args['shortcode_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SPLC_FREE_Shortcoder::instance( $key, $params );
					}
				}

				// Once editor setup for gutenberg and media buttons
				if ( class_exists( 'SPLC_FREE_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
					foreach ( self::$shortcode_instances as $instance ) {
						if ( ! empty( $instance['show_in_editor'] ) ) {
							SPLC_FREE_Shortcoder::once_editor_setup();
							break;
						}
					}
				}
		...
		public static function textdomain() {
			load_textdomain( 'splogocarousel', self::$dir . '/languages/' . get_locale() . '.mo' );
		}
		...
		public static function add_admin_enqueue_scripts() {
			...
			if ( $wpscreen->id === 'tools_page_splogocarousel-welcome' ) {
				self::$enqueue = true;
			}
		...
		public static function add_admin_body_class( $classes ) {
			...
			if ( apply_filters( 'splogocarousel_fa4', false ) ) {
				$classes .= 'splogocarousel-fa5-shims';
			}
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field_type ) ) {
				...
				echo '<div class="splogocarousel-field splogocarousel-field-' . $field_type . $is_pseudo . $class . $visible . '"' . $depend . '>';

				if ( ! empty( $field['fancy_title'] ) ) {
					echo '<div class="splogocarousel-fancy-title">' . $field['fancy_title'] . '</div>';
				}

				if ( ! empty( $field['title'] ) ) {
					echo '<div class="splogocarousel-title">';
					echo '<h4>' . $field['title'] . '</h4>';
					echo ( ! empty( $field['subtitle'] ) ) ? '<div class="splogocarousel-subtitle-text">' . $field['subtitle'] . '</div>' : '';
					echo '</div>';
				}
