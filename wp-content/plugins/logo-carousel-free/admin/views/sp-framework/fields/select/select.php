			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
					...
					if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
						...
						echo '<select name="' . $field_name . '" class="splogocarousel-hide-select hidden"' . $multiple_attr . $field_attr . '>';
					...
					foreach ( $options as $option_key => $option ) {
						...
						if ( isset( $option['text'] ) ) {
							...
							echo '<option ' . $pro_only . ' value="' . esc_attr( $option_key ) . '" ' . esc_attr( $selected ) . '>' . esc_html( $option['text'] ) . '</option>';
						} else {
							echo '<option value="' . esc_attr( $option_key ) . '" ' . esc_attr( $selected ) . '>' . esc_attr( $option ) . '</option>';
						}
						// }
